...
const Dashboard = () => {
  ...
  const [selectedSynergyLaneFilter, setSelectedSynergyLaneFilter] = useState('all'); // 'all', 'dark', 'farm', 'mid', 'abyssal', 'support'
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('/Game_History.csv');
      ...
      if (picks1.length === 5) {
        ...
        picks1.forEach((character, index) => {
          ...
          if (!characterStats[character]) {
            characterStats[character] = { pickCount: 0, winCount: 0, banCount: 0 };
          }
  ...
  const getSortedCharacterStats = () => {
    const statsArray = Object.entries(filteredStats).map(([character, stats]) => {
      const winRate = stats.pickCount > 0 ? (stats.winCount / stats.pickCount * 100).toFixed(2) : 0;
  ...
  const getSortedSynergies = () => {
    ...
    if (selectedSynergyLaneFilter !== 'all' && synergyLaneFilterMode !== 'none') {
  ...
  const getTopByWinRate = (minGames = 5, limit = 10) => {
    return Object.entries(filteredStats)
      .map(([character, stats]) => ({
        name: character,
        winRate: stats.pickCount > 0 ? (stats.winCount / stats.pickCount * 100).toFixed(2) : 0,
  ...
  const getLaneCharacters = (lane, sortBy = 'winRate', minGames = 3, limit = 10) => {
    if (!laneStats[lane]) return [];
    ...
    <div className="dashboard-container">
      ...
      {activeTab === 'counters' && (
        <div className="search-container">
          <input 
            ...
            className="search-input"
            placeholder={
              counterSearchMode === 'pick' ? "Search for a hero to pick..." :
              counterSearchMode === 'against' ? "Search for a hero to counter..." :
              "Search for any hero..."
            }
            value={searchQuery}
            onChange={handleSearchChange}
      ...
      {searchQuery && activeTab !== 'counters' && (
        <div className="search-results">
          Found {Object.keys(filteredStats).length} characters matching "{searchQuery}"
        </div>
      )}
      ...
      {searchQuery && activeTab === 'counters' && (
        <div className="search-results">
          ...
          {counterSearchMode === 'pick' ? ' "Pick This" ' : 
           counterSearchMode === 'against' ? ' "Against" ' : 
           ' either column '} 
          includes "{searchQuery}"
          {laneFilterMode !== 'none' && selectedLaneFilter !== 'all' && (
            <span>
              {' '}and {laneFilterMode === 'pickLane' ? '"Pick Lane"' : 
                       laneFilterMode === 'againstLane' ? '"Against Lane"' : 
                       'either lane'} is {formatLaneName(selectedLaneFilter)}
            </span>
          )}
        </div>
      )}
          ...
          <div className="chart-container">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <YAxis label={{ value: 'Pick Rate (%)', angle: -90, position: 'insideLeft' }} />
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th 
                    className={`sortable ${getSortClass('pickCount')} text-right`} 
                  ...
                  <th 
                    className={`sortable ${getSortClass('winCount')} text-right`} 
                  ...
                  <th 
                    className={`sortable ${getSortClass('banCount')} text-right`} 
      ...
      {activeTab === 'lanes' && (
        <div className="content-card">
          ...
          <div className="lane-tabs">
            <div 
              className={`lane-tab ${selectedLane === 'dark' ? 'lane-tab-active' : ''}`}
              onClick={() => setSelectedLane('dark')}
            >
              Dark Lane
            </div>
            <div 
              className={`lane-tab ${selectedLane === 'farm' ? 'lane-tab-active' : ''}`}
              onClick={() => setSelectedLane('farm')}
            >
              Farm Lane
            </div>
            <div 
              className={`lane-tab ${selectedLane === 'mid' ? 'lane-tab-active' : ''}`}
              onClick={() => setSelectedLane('mid')}
            >
              Mid Lane
            </div>
            <div 
              className={`lane-tab ${selectedLane === 'abyssal' ? 'lane-tab-active' : ''}`}
              onClick={() => setSelectedLane('abyssal')}
            >
              Abyssal Lane
            </div>
            <div 
              className={`lane-tab ${selectedLane === 'support' ? 'lane-tab-active' : ''}`}
          ...
          <h3 className="section-title">Top Characters for {formatLaneName(selectedLane)}</h3>
          <div className="chart-container">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={getLaneCharacters(selectedLane, 'winRate').map(char => ({
                  name: char.name,
      ...
      {activeTab === 'synergies' && (
        <div className="content-card">
          <h2 className="section-title">Character Synergies (Pairs with High Win Rates)</h2>
          
          {/* Lane Filter Controls for Synergies */}
          <div style={{ marginBottom: '15px', display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ marginRight: '20px', display: 'flex', alignItems: 'center' }}>
              <label style={{ marginRight: '10px', fontWeight: 'bold' }}>Lane Filter:</label>
              <select 
            ...
            {synergyLaneFilterMode !== 'none' && (
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <label style={{ marginRight: '10px', fontWeight: 'bold' }}>Select Lane:</label>
                <select 
          ...
          {searchQuery && (
            <div className="search-results">
              Found {getSortedSynergies().length} synergies matching "{searchQuery}"
              {synergyLaneFilterMode !== 'none' && selectedSynergyLaneFilter !== 'all' && (
                <span>
                  {' '}and {
                    synergyLaneFilterMode === 'char1Lane' ? 'first character lane' : 
                    synergyLaneFilterMode === 'char2Lane' ? 'second character lane' : 
                    synergyLaneFilterMode === 'both' ? 'both characters in same lane' :
                  ...
                  <tbody>
                    {getSortedSynergies()
                      ...
                      .map((pair, index) => {
                        ...
                        const supportChar = pair.lane1 === 'support' ? pair.char1 : pair.char2;
                        const farmChar = pair.lane1 === 'farm' ? pair.char1 : pair.char2;
                  ...
                  <tbody>
                    {getSortedSynergies()
                      ...
                      .map((pair, index) => {
                        ...
                        const midChar = pair.lane1 === 'mid' ? pair.char1 : pair.char2;
                        const darkChar = pair.lane1 === 'dark' ? pair.char1 : pair.char2;
      ...
      {activeTab === 'counters' && (
        <div className="content-card">
          <h2 className="section-title">Counter Picks (Characters that Win Against Specific Opponents)</h2>
            ...
            <table className="data-table">
              ...
              <tbody>
                {getSortedCounters()
                  ...
                  .map((counter, index) => (
                    <tr key={index}>
                      <td>{counterSearchMode === 'pick' || counterSearchMode === 'both' 
                        ? highlightText(counter.pick) 
                        : counter.pick}
                      </td>
                      <td>{formatLaneName(counter.pickLane)}</td>
                      <td>{counterSearchMode === 'against' || counterSearchMode === 'both' 
          ...
          <h3 className="section-title" style={{ marginTop: '20px' }}>Perfect Counters (100% Win Rate)</h3>
          <div style={{ overflowY: 'auto', maxHeight: '300px' }}>
            <table className="data-table-sm">
              ...
              <tbody>
                {getSortedCounters()
                  .filter(counter => parseFloat(counter.winRate) === 100 && counter.games >= 3)
                  .map((counter, index) => (
                    <tr key={index}>
                      <td>{counterSearchMode === 'pick' || counterSearchMode === 'both' 
                        ? highlightText(counter.pick) 
          ...
          <div className="chart-container">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="name" angle={-45} textAnchor="end" height={70} />
                <YAxis label={{ value: 'Ban Rate (%)', angle: -90, position: 'insideLeft' }} />
